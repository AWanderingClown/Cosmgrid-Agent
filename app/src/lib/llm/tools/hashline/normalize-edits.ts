// 原样移植自 hashline-core/src/normalize-edits.ts——把工具参数（宽松的原始输入）
// 收敛成 HashlineEdit 判别联合，缺字段/坏 op 直接抛错，不让脏数据进 apply 流程。
...
export function normalizeHashlineEdits(rawEdits: RawHashlineEdit[]): HashlineEdit[] {
  return rawEdits.map((rawEdit, index) => {
    ...
    switch (edit.op) {
      ...
      default:
        throw new Error(`Edit ${index}: unsupported op "${String(edit.op)}". Use op/pos/end/lines.`);
    }
  });
}
