...
>
> **2026-07-17 已接线主链路**（原为零调用骨架，4 断点 + 2 事件源全通）：
> - 事件写入：stream-finalization（outcome_failed / outcome_needs_user）、CheckpointDialogs
>   保存（checkpoint_failed）、prompt-compression 摘要落库（summary_dropped）
> - 消费：`pipeline.ts` runPlaybookPipeline（stream-finalization 每轮后台 fire-and-forget +
>   checkpoint 保存后 project 级触发）；第一版纪律见 pipeline.ts 头注释（requiresConfirm →
>   status='candidate' 候选区，不动老条目）
> - 注入：chat-memory-preamble 融合 assemblePlaybookContext（语义 hits 优先 + 加权补足，上限 10）
> - 反馈：WorkPanel PlaybookPanel 赞/踩 + recordMemoriesUsed（本轮注入自动 touch last_used）
> - 防重：Curator exact-title 查重索引含 active+candidate（复检抓过"candidate 不可见 →
>   每轮重复落新行无限膨胀"的 HIGH，勿回退）；manual 手建条目绝不被自动 supersede
...
## 测试覆盖（2026-07-17 接线时真实补齐——此前 __tests__ 目录为空，下表原是计划非实况）

| 模块 | case 数 | 关键断言 |
|---|---|---|
| `reflector.test.ts` | 5 | 5 类事件 → candidate 提炼规则 + 坏 payload 不中断 |
| `curator.test.ts` | 9 | skip（重复，含 candidate 状态）/ supersede（相似，manual 豁免）/ mark_disputed（矛盾）/ create 自动入/confirm / archived+harmful 不参与查重 |
| `context-assembler.test.ts` | 4 | tags 加权 / harmful 降权 / maxChars 截断 / confidence 加权 |
| `pipeline.test.ts` | 8 | 落库纪律（candidate/active/supersede/不动老条目）/ 断链回归 / 幂等回归（二次消费 0 落库）/ 旁路吞错 |
| `stream-finalization.test.ts`（chat 层） | +3 | failed 写事件先于 pipeline / passed 只消费不写 / projectId=null 全跳过 |
