...
// 模块级缓存：normalize(modelId) → 输出上限。null 表示还没加载过。
let limitMap: Map<string, number> | null = null;
let loadPromise: Promise<void> | null = null;

function loadFromStorage(): Map<string, number> | null {
  try {
...
export function parseModelsDev(json: unknown): Map<string, number> {
  ...
  for (const provider of Object.values(json as Record<string, unknown>)) {
    ...
    for (const [modelId, meta] of Object.entries(models as Record<string, unknown>)) {
      const output = (meta as { limit?: { output?: unknown } })?.limit?.output;
      ...
      for (const k of keys) {
        ...
        if (!map.has(nk)) map.set(nk, output);
      }
    }
  }
  return map;
...
export async function ensureModelLimitsLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    ...
    try {
      ...
      if (res.ok) {
        const parsed = parseModelsDev(await res.json());
        if (parsed.size > 0) {
          limitMap = parsed;
      ...
      // 网络失败：保留 cached（或留空，由 resolve 用 CEILING 兜底）
    } finally {
...
/** 仅供测试：直接注入限制表，绕过网络。 */
export function __setLimitMapForTest(map: Map<string, number> | null): void {
  limitMap = map;
  loadPromise = null;
}
