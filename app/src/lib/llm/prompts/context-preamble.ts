...
// 2026-07-10 OMO-5：模型族判断逻辑单一来源化，抽到 model-variant.ts 的
// resolveModelVariant——这里不再自己维护一份关键词表（原判断逻辑 + 调研注释
// 已原样搬过去，见该文件）。CosmGrid.md 是所有模型共用的核心提示，不适合把
// 这段只对部分模型有效的话塞进去，所以继续按 driverLabel 单独判断、命中才加。
...
export function buildDomesticModelReminder(driverLabel: string | null | undefined): string | null {
  if (resolveModelVariant(driverLabel) !== "domestic") return null;
  return [
