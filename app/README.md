...
| API Key 存储 | 系统凭据库（macOS Keychain / Windows Credential Manager / Linux Secret Service），经 Rust `keyring` 命令访问；旧 `cosmgrid-keys.json` 只作为升级迁移来源 |
...
│   │   ├── keystore.ts           #   API Key 存取（系统 keychain + 旧明文文件迁移）
...
- ✅ **v0.8** 多模型对弈（出方案/反驳/裁判同台 + ChatPage 对话内触发与自动建议）
